#!/usr/bin/env node
// Kleiner, eigenstaendiger Webserver fuer das Urban-Heroes-Challenge-Leaderboard.
// Fragt bei JEDEM Seitenaufruf live Databricks per REST ab (kein Caching, kein
// MCP-Tool-Call, keine lokale Abhaengigkeit) - Klarnamen laufen nur hier durch,
// nie sichtbar durch ein LLM. Zugriff per einfachem Basic-Auth-Passwort
// geschuetzt, damit die Seite nicht komplett offen im Netz haengt.
...
const AUTH_USER = process.env.LB_USER || 'urbanheroes';
const AUTH_PASS = process.env.LB_PASSWORD || '';
...
function checkAuth(req) {
  if (!AUTH_PASS) return true; // kein Passwort gesetzt -> Auth-Check uebersprungen (nicht empfohlen)
...
async function computeLeaderboard() {
  if (!HOST || !TOKEN || !WAREHOUSE) {
    throw new Error('Databricks-Zugangsdaten fehlen (DATABRICKS_HOST / DATABRICKS_TOKEN / DATABRICKS_HTTP_PATH als Secrets setzen).');
  }
...
.refresh-hint { font-size: 0.72rem; color: var(--muted); text-align: center; margin-top: 10px; }
...
<div class="wrap">
  ...
  <div class="stamp"><span class="dot"></span>Live-Stand ${stamp}</div>
  ${podiumHtml}
  ...
  <p class="refresh-hint">Seite neu laden f&uuml;r den aktuellsten Stand.</p>
</div>
`;
}

function errorPage(message) {
  return `<!doctype html><title>Fehler</title><meta name="viewport" content="width=device-width, initial-scale=1">
  <body style="font-family: -apple-system, sans-serif; max-width: 600px; margin: 60px auto; padding: 0 20px; color:#1c1c1c;">
  <h1 style="font-size:1.3rem;">Leaderboard konnte nicht geladen werden</h1>
  <p style="color:#6b6b70;">${escapeHtml(message)}</p>
  </body>`;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (!checkAuth(req)) {
    res.writeHead(401, { 'WWW-Authenticate': 'Basic realm="Leaderboard"', 'Content-Type': 'text/plain' });
    res.end('Zugang benoetigt.');
    return;
  }
  ...
  try {
    ...
    res.end(errorPage('Die Databricks-Abfrage ist fehlgeschlagen. Bitte in ein paar Minuten erneut versuchen.'));
  }
});

server.listen(PORT, () => {
  console.log(`Leaderboard-Server laeuft auf Port ${PORT}`);
});
